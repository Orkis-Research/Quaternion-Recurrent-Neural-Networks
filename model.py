...
from   quaternion_models import QRN, MultiLayerQRN
from   quaternion_models import BidirectionalQRN as BiQRN
...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, in_channels=None, out_channels=None, kernel_size=None,
        ...
        if rnn_type not in {'CRU', 'CGRU', 'CLSTM', 'QGRU', 'QLSTM', 'QRNN'}:
            self.encoder = nn.Embedding(ntoken, ninp)
        elif rnn_type not in {'QGRU', 'QLSTM', 'QRNN'} :
            self.encoder = nn.Embedding(ntoken, 2 * ninp)
        else:
            self.encoder = nn.Embedding(ntoken, 4 * ninp)
        # II - RNN part
        # II.1 {'LSTM', 'GRU', 'CRU', 'CGRU', 'CLSTM'}
        if rnn_type in {'LSTM', 'GRU', 'CRU', 'CGRU', 'CLSTM', 'QGRU', 'QLSTM', 'QRNN'}:
            ...
            # II.1.c Quaternion-valued models
            elif rnn_type in {'QGRU', 'QLSTM', 'QRNN'}:
                if ((not self.bidirectional) and nlayers == 1):
                    RNN = QRN    
                elif (self.bidirectional and nlayers == 1):
                    RNN = BiQRN  
        ...
        # II.2 {'RNN_TANH': 'tanh', 'RNN_RELU': 'relu'}
            ...
            try:
                ...
                raise ValueError("""An invalid option for `--model` was supplied,
                                 options are ['LSTM', 'GRU', 'CRU', 'CGRU', 'CLSTM', RNN_TANH', 'RNN_RELU', 'QGRU', 'QLSTM', or 'QRNN']""")
            self.rnn = nn.RNN(ninp, nhid, nlayers, nonlinearity=nonlinearity, dropout=dropout)
        
        
        # III - Classification part
        # III.1 Real-Valued Models
        if rnn_type not in {'CRU', 'CGRU', 'CLSTM', 'QGRU', 'QLSTM', 'QRNN'}: #which means if the rnn is not complex
        ...
        # III.2 Complex-Valued Models
            ...
            nb_directions = 2 if self.bidirectional else 1
            if self.operation == 'linear':
                self.decoder = nn.Linear(4 * nhid * nb_directions, ntoken)
    ...
    def init_hidden(self, bsz):
        ...
        elif   self.rnn_type in {'QLSTM'} and self.operation == 'linear':
            nhid = self.nhid * 4
            if self.rnn_type == 'QLSTM':
                v = (Variable(weight.new(self.nlayers, bsz, nhid).zero_()),
                     Variable(weight.new(self.nlayers, bsz, nhid).zero_()))
                if   nb_directions == 1 and self.nlayers == 1:
                    return v
        ...
        elif self.rnn_type in {'QGRU','QRNN'}:
            if not self.bidirectional:
                return Variable(weight.new(    self.nlayers, bsz, self.nhid * 4).zero_())
            else: # nb_directions here is going to be = 2
                if self.nlayers > 1:
                    return Variable(weight.new(self.nlayers, nb_directions, bsz, self.nhid * 4).zero_())
                elif self.nlayers == 1:
                    return Variable(weight.new(              nb_directions, bsz, self.nhid * 4).zero_())
        else:
            return Variable(weight.new(self.nlayers * nb_directions, bsz, self.nhid).zero_())
