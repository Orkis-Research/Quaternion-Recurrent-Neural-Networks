...
class ConvolutionalCRN(Module):
    r"""Applies a 1 layer complex convolutional recurrent unit (CRU) RNN to an input sequence.
    ...
    def __init__(self, in_channels, out_channels, kernel_size, rec_kernel_size,
                 stride, dilation, padding, rnn_type='CRU', bias=True, dropout=0,
                 reverse=False, init_criterion='glorot', weight_init='unitary',
                 w_mod_init='orthogonal', modulation_activation='softmax',
                 dropout_type='regular', seed=None, **kwargs):

        super(ConvolutionalCRN, self).__init__(**kwargs)
        ...
        if   self.rnn_type in {'CRU'}:
            gate_channels    =    2 * self.out_channels
        elif self.rnn_type in {'CGRU'}:
        ...
        self.w_ih_real       =    Parameter(torch.Tensor(*self.w_ih_shape))
        self.w_ih_imag       =    Parameter(torch.Tensor(*self.w_ih_shape))
        self.w_hh_real       =    Parameter(torch.Tensor(*self.w_hh_shape))
        self.w_hh_imag       =    Parameter(torch.Tensor(*self.w_hh_shape))
        if self.rnn_type in {'CRU'}:
            w_ih_mod_shape   =    (self.out_channels, self.in_channels  * 2) + tuple((self.kernel_size,))
            w_hh_mod_shape   =    (self.out_channels, self.out_channels * 2) + tuple((self.rec_kernel_size,))
            self.w_ih_mod    =    Parameter(torch.Tensor(*w_ih_mod_shape))
            self.w_hh_mod    =    Parameter(torch.Tensor(*w_hh_mod_shape))
        
        if self.bias:
            self.b_ih         =    Parameter(torch.Tensor(gate_channels * 2))
            self.b_hh         =    Parameter(torch.Tensor(gate_channels * 2))
            if self.rnn_type in {'CRU'}:
                self.b_ih_mod =    Parameter(torch.Tensor(self.out_channels))
                self.b_hh_mod =    Parameter(torch.Tensor(self.out_channels))
        else:
            self.register_parameter('bias_ih',        None)
            self.register_parameter('bias_hh',        None)
            if self.rnn_type in {'CRU'}:
    ...
    def reset_parameters(self):
        fargs = [self.winit, self.rng, self.init_criterion]
        affect_conv_init(self.w_ih_real,       self.w_ih_imag,        self.kernel_size,        *fargs)
        affect_conv_init(self.w_hh_real,       self.w_hh_imag,        self.rec_kernel_size,    *fargs)
        if self.rnn_type in {'CRU'}:
            modfargs = [self.kernel_size, self.wmodinit, self.rng, self.init_criterion]
            realops.affect_conv_init(self.w_ih_mod, *modfargs)
            realops.affect_conv_init(self.w_ih_mod, *modfargs)
        if self.bias:
            self.b_ih.data.zero_()
            self.b_hh.data.zero_()
...
class BidirectionalConvolutionalCRN(Module):

    def __init__(self, in_channels, out_channels, kernel_size, rec_kernel_size,
                 stride, dilation, padding, rnn_type='CRU', bias=True, dropout=0,
                 dropout_type='regular', init_criterion='glorot',
                 weight_init='unitary', seed=None, **kwargs):

        super(BidirectionalConvolutionalCRN, self).__init__()
        self.rnn_type    =  rnn_type
        arguments = {
            ...
            'dropout_type'   : dropout_type,   'init_criterion'  : init_criterion,
            'weight_init'    : weight_init,    'seed'            : seed
        }
        self.forwardcrn   =  ConvolutionalCRN(reverse=False, **dict(**arguments, **kwargs))
        self.backwardcrn  =  ConvolutionalCRN(reverse=True,  **dict(**arguments, **kwargs))
...
class MultiLayerConvolutionalCRN(Module):
    """heavily inspired from:
       https://github.com/pytorch/benchmark/blob/master/benchmarks/lstm_variants/container.py
    """
    def __init__(self, in_channels, bidirectional=False, rnn_type='CRU', out_channels_list=(64, 64), **kwargs):
        super(MultiLayerConvolutionalCRN, self).__init__()
        self.bidirectional = bidirectional
        self.rnn_type      = rnn_type
        layers = []
        ...
        for out_channels in out_channels_list[:-1]:
            if self.bidirectional:
                layer = BidirectionalConvolutionalCRN(
